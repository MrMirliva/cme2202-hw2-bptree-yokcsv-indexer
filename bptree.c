...
void split_node(BPNode *parent, int i)
{
    ...
    // 1) Orijinal anahtar sayısı ve bölme indeksi
    int orig = child->num_keys;
    int t = orig / 2;

    if (child->is_leaf)
    {
        // (Aynı leaf bölme kodu olduğu gibi kalıyor…)
        new_node->num_keys = child->num_keys - t;
    ...
    {
        // 2) Promote edilecek anahtar
        char *promoted = child->keys[t];

        // 3) Sağ düğüme anahtarları ve çocuk pointer’larını taşı
        ...
        // --- burayı düzelttik: son çocuk c[orig] olmalı ---
        new_node->children[new_node->num_keys] = child->children[orig];
        child->children[orig] = NULL;

        // 4) Soldaki düğümün anahtar sayısını güncelle
        child->num_keys = t;

        // 5) Parent’ı kaydırıp güncelle
        for (int j = parent->num_keys; j > i; --j)
        {
            parent->keys[j] = parent->keys[j - 1];
            parent->children[j + 1] = parent->children[j];
...
static void insert_nonfull(BPNode *node, Record *rec)
{
    ...
    if (!node->is_leaf)
    {
        ...
        if (pos > BP_DEGREE || node->children[pos] == NULL) {
            fprintf(stderr,
        ...
        {
            split_node(node, pos);
            // Bölme sonrası yeni doğru çocuğu seç
...
/**
 * Main insertion entry: handles root split if gerekli,
 * then calls insert_nonfull().
 */
